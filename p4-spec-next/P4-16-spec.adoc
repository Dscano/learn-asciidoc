...
example), whose behavior is described in <<_very_simple_switch_architecture_description>>. The
...
TODO: Put correct numbered list here.
...
described in <<_operations_on_error_types>>.
...
used in a table's key property (described in <<_table_properties>>).
...
<<_operations_on_match_kind_types>>.
...
<<_calling_convention_call_by_copy_in_copy_out>>).
...
literals, described in <<_string_literals>>.  For example,
...
compile-time known value (see <<_compile_time_known_values>>) that is a
...
<<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>> for additional details.
...
described in <<_operations_on_fixed_width_bit_types_unsigned_integers>>.
...
in <<_operations_on_fixed_width_signed_integers>>.
...
<<_operations_on_variable_size_bit_types>>.
...
are described in <<_operations_on_arbitrary_precision_integers>>.  The following example
...
<<_calling_convention_call_by_copy_in_copy_out>> for more details on directionless
parameters.
...
types. For additional examples of literals see <<_literal_constants>>.

TODO: Learn how to insert proper AsciiDoc table here.
...
types are described in <<_synthesized_data_types>>: set types and
...
described in <<_annotations>>.

Operations on `enum` values are described in
<<_operations_on_enum_types>>.
...
and `setInvalid()`, as described in <<_operations_on_headers>>.
...
in <<_data_extraction>>. The successful execution of
...
As demonstrated by a code example in <<_variable_width_extraction>>,
...
on header stacks are discussed in <<_operations_on_header_stacks>>.
...
Operations on header stacks are described in <<_operations_on_header_stacks>>.
...
Operation on header unions are described in <<_operations_on_header_unions>>.
...
<<_operations_on_tuple_expressions>>.
...
<<_variable_width_extraction>>) on packets only supports a single `varbit`
field in a header.
...
(see <<_select_expressions>>), matching any value in this range. Set
...
(see <<_data_extraction>> and <<_deparsing>>). Here
...
implementation of all the `abstract` methods (see <<_instantiating_objects_with_abstract_methods>>).
...
constructors (see <<_parameterization>>).
...
<<_example_architecture_description>>.
...
See <<_appendix_p4_grammar>> for the complete P4 grammar.
...
in <<_select_expressions>>), which may be used only in parsers.
...
implicit casting from an `enum` to its fixed-width (signed or unsigned) integer type is also supported (see <<_implicit_casts>>):
...
underlying type cannot be determined at compile-time (<<_compile_time_size_determination>>).
...
compile-time (<<_compile_time_size_determination>>).
...
in <<_implicit_casts>> for details on how the `0` in this
...
* Extraction of a set of contiguous bits, also known as a slice,
  ...
  (which do not need to be identical) must be numeric (<<_numeric_types>>).
...
* Extraction of a set of contiguous bits, also known as a slice,
  ...
  must be numeric (<<_numeric_types>>).
...
* Arithmetic shift left and right denoted by `<<` and `>>`. These operations produce an `int` result.
  ...
  The expression `a << b` is equal to stem:[a \times 2^b] while `a >> b`
  is equal to stem:[\lfloor{a / 2^b}\rfloor].
...
to fixed-width types in certain situations---see <<_casts>>.
...
* casts of a key-value list to a struct type or a header type (see <<_operations_on_structure_valued_expressions>>)
...
<<_select_expressions>>, since select labels are compared against the selected
...
The type of a tuple expression is a tuple type (<<_tuple_types>>).
...
structure-valued expression (see <<_operations_on_structure_valued_expressions>>):
...
explicitly will be given their default value (see <<_initializing_with_default_values>>).
...
The infix operator `&&&` takes two arguments of the same numeric type (<<_numeric_types>>),
...
to unify the argument types in certain situations (<<_implicit_casts>>).
...
The infix operator `..` takes two arguments of the same numeric type `T` (<<_numeric_types>>),
...
with a tuple expression, as discussed in <<_operations_on_tuple_expressions>>, or
with a structure-valued expression, as described in
<<_operations_on_structure_valued_expressions>>.  Both of these cases must initialize all
...
See <<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>>
...
expression (see <<_operations_on_tuple_expressions>>) --- the tuple fields are assigned to the
header fields in the order they appear --- or with a structure-valued
expression (see <<_operations_on_struct_types>>).  When initialized the header
...
greater than or equal to `hs.size` results in an undefined value.  See <<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>> for more
details.
+
The `index` is an expression that must be of numeric types (<<_numeric_types>>).
...
The calling convention is copy-in/copy-out (<<_calling_convention_call_by_copy_in_copy_out>>).  For generic functions the type arguments
...
functions as described in <<_casts>>.  The types for all
...
* Using instantiations, described in <<_instantiations>>.
...
Constructors are evaluated entirely at compilation time (see <<_p4_abstract_machine_evaluation>>). In consequence, all constructor arguments
...
As mentioned in <<_operations_on_header_stacks>>, any reference to an element of
...
suitable type using the syntax `...` (see <<_default_values>>).  A value
...
same way as described for direction `out` parameters in <<_calling_convention_call_by_copy_in_copy_out>>). The language places few restrictions on
...
or `extern` types must be declared using instantiations (see <<_instantiations>>).
...
Instantiations are always executed at compilation time (<<_compile_time_known_values>>).
...
In addition, parsers support a `transition` statement (<<_transition_statements>>).
...
<<_calling_convention_call_by_copy_in_copy_out>> for details on copy-out behavior.
...
performed after the execution of the `exit` statement.  See <<_calling_convention_call_by_copy_in_copy_out>> for details on copy-out behavior.
...
* `table.apply().action_run`, which can only appear as the expression
  of a `switch` statement (see <<_switch_statement>>), and when
  it appears there, it must be the entire expression.
* Any expression containing `table.apply().hit` or
  `table.apply().miss` (see <<_match_action_unit_invocation>>), which can be
...
** an expression to calculate a table key (see <<_match_action_unit_execution_semantics>>).
...
statement does not complete its execution.  See <<_expression_evaluation_order>> for the order of evaluation of the parts of an
...
<<_expressions>>, except it does not include any cases that can
...
<<_match_action_unit_invocation>>).  All switch labels must be names of
...
`switch` expression (see <<_implicit_casts>>).  Switch
...
building parameterized parsers, see <<_parameterization>>.
...
<<_a_complete_very_simple_switch_program>>.
...
- Method calls, which serve several purposes:
  ...
  * Invoking methods (e.g., extracting data out of packets or computing checksums) and other parsers (see <<_sub_parsers>>), and
- Conditional statements,
- Transitions to other states (discussed in <<_transition_statements>>).
...
Some targets may support parser value sets; see <<_parser_value_sets>>. Given
...
<<_l_values>>) of type `header` with a fixed width. If
...
The TCP options example from <<_operations_on_header_unions>> also illustrates how `lookahead` can be used:
...
direct invocation (see <<_direct_type_invocation>>).
...
can be used to build parameterized control blocks, see <<_parameterization>>.
...
- `largest_priority_wins` - Only useful for some tables with the
  `entries` property.  See <<_entries>> for details.
- `priority_delta` - Only useful for some tables with the `entries`
  property.  See <<_entries>> for details.
...
constant that describes the algorithm used to perform the lookup (see <<_the_match_kind_type>>).
...
<<_match_action_unit_execution_semantics>>.
...
- a `ternary` match kind on a key field means that the field in the
  ...
  mask expressions, as described in <<_masks>>: a key field
...
An example specifying entries for a table is given in <<_entries>>.
...
specified for a table with no key (see <<_keys>>).
...
property (see <<_default_action>>), independently of whether
...
provide a field for each key in the table keys (see <<_table_properties>>). The table key type must match the type of the
...
The following example is the same as the first example in <<_entries>>, except for the definition of table `t_exact_ternary`
shown below.
...
to direct invocation (see <<_direct_type_invocation>>).
...
All the behavior above also applies to direct invocation (see <<_direct_type_invocation>>).
...
See <<_annotations_controlling_naming>> for details of `@name`
annotations.
...
example see the Very Simple Switch declaration from <<_very_simple_switch_architecture>>.
...
- Expressions evaluating to a list type, where all elements are
  compile-time known values.
- Instances constructed by instance declarations (<<_instantiations>>) and constructor invocations.
...
expression to be annotated with a `@name` annotation (<<_control_plane_api_annotations>>), as in the following example:
...
[#fig-evalmultiple]
image::evalmultiple.png[]

==== Annotations controlling naming

Control plane-related annotations (<<_control_plane_api_annotations>>) can alter the names exposed to
...
first example in <<_control_plane_names>>.  One might be tempted to
...
executed. For example, in the Simple Switch example from <<_very_simple_switch_architecture>> the execution flow goes `Parser->Pipe->Deparser`.
...
<<_appendix_p4_grammar>> for details.
...
set of "standard" annotations in <<_appendix_P4_reserved_annotations>>.
...
removes its fully-qualified name (<<_control_plane_names>>).  This annotation
...
The `@atomic` annotation, described in <<_concurrency_model>>
can be used to enforce the atomic execution of a code block.

==== Value set annotations

The `@match` annotation, described in <<_select_expressions>>, is used
...
ways (subject to a few limitations -- see <<_calling_convention_call_by_copy_in_copy_out>>).  Since externs are
...
* Added header stack expressions (<<_header_stack_expressions>>).
* Allow casts from a type to itself (<<_casts>>).
* Added an invalid header or header union expression `+{#}+` (<<_operations_on_headers>> and <<_operations_on_header_unions>>).
* Added a concept of numeric values (<<_numeric_types>>).
* Added a section on operations on extern objects (<<_operations_on_extern_objects>>).
...
* Clarified types `bit<W>`, `int<W>`, and `varbit<W>` encompass the case where the width is a compile-time known expression evaluating to an appropriate integer (<<_unsigned_integers_bit_strings>>, <<_signed_integers>>, <<_dynamically_sized_bit_strings>>).
* Clarified restrictions for parameters with default values (<<_calling_convention_call_by_copy_in_copy_out>>).
* Added optional trailing commas (<<_optional_trailing_commas>>).
* Clarified the scope of parser namespaces (<<_parser_declarations>>).
* Specified that algorithm for generating control-plane names for keys is optional (Section [#sec-cp-keys]).
* Clarified types of expressions that may appear in `select` (<<_select_expressions>>).
* Added description of semantics of the core.p4 match kinds (<<_keys>>).
* Explicitly disallow overloading of parsers, controls, and packages (<<_extern_objects>>).
* Clarified implicit casts present in select expressions (<<_select_expressions>>).
* Clarified that slices can be applied to arbitrary-precision integers (<<_operations_on_arbitrary_precision_integers>>).
* Clarified that direct invocation is not possible for objects that have constructor arguments (<<_direct_type_invocation>>).
* Added comparison for tuples as a legal operation (<<_operations_on_tuple_expressions>>).
* Clarified the behavior of `lookahead` on header-typed values (<<_lookahead>>).
* Added `static_assert` function (<<_static_assertions>>).
* Clarified semantics of ranges where the start is bigger than the end (<<_ranges>>).
* Allow ranges to be specified by serializable enums (<<_ranges>>).
* Specified type produced by the `*sizeInB*` methods (<<_compile_time_size_determination>>).
* Added section with operations on `match_kind` values (<<_operations_on_match_kind_types>>).
* Renamed infinite-precision integers to arbitrary-precision integers (<<_arbitrary_precision_integers>>).
* compiler-inserted `default_action` is not `const` (Section [#sec-tables]).
* Clarified the restrictions on run time for tables with `const entries` (<<_entries>>).
* renamed list expressions to tuple expressions
* Added `list` type (<<_list_types>>).
* Defined `entries` table property without `const`, for entries
  ...
  later change them or add to them (<<_entries>>).
* Clarified behavior of table with no `key` property, or if its list
  of keys is empty (<<_keys>>).

=== Summary of changes made in version 1.2.3, released July 11, 2022.

* Extended `minSizeInBits` and `minSizeInBytes` to apply to more expressions (<<_compile_time_size_determination>>).
* Added support for `maxSizeInBits` and `maxSizeInBytes` (<<_compile_time_size_determination>>).
* Added support for empty lists of const entries in tables (<<_entries>>).
* Added support for `switch` statements in actions (Section [#sec-actions]).
* Added support for direct invocation of controls and parsers (<<_parameterization>>).
* Added parser `value_set` to list of control-plane visible names (<<_control_plane_names>>).
* Added `match_kind` as a base type (<<_the_match_kind_type>>).
* Removed structure initializers as they are subsumed by structure-valued expressions (<<_operations_on_structure_valued_expressions>>).
* Specified operations on values typed as type variables (<<_operations_on_types_that_are_type_variables>>).
* Clarified semantics of compile-time known values (<<_compile_time_known_values>>).
* Clarified semantics of directionless parameters (<<_calling_convention_call_by_copy_in_copy_out>>).
* Clarified semantics of arbitrary precision integers (<<_arbitrary_precision_integers>>).
* Clarified semantics of bit slices, shifts, and concatenation (<<_operations_on_fixed_width_bit_types_unsigned_integers>>).
* Clarified semantics of optional parameters (<<_optional_parameters>>).
* Clarified restrictions on extern method and function invocation (<<_appendix_restrictions_on_compile_time-and_run_time_calls>>).
* Clarified semantics of implicit casts (<<_implicit_casts>>).

=== Summary of changes made in version 1.2.2, released May 17, 2021

* Added support for accessing tuple fields (<<_operations_on_tuple_expressions>>).
* Added support for generic structures (<<_type_specialization>>).
* Added support for integers, ``enum``s, and ``error``s in `switch` statements (<<_switch_statement>>).
* Added support for additional enumeration types (<<_enumeration_types>>).
* Added support for abstract methods (<<_abstract_methods>>).
* Added support for conditional statements and empty statements in parsers (Section [#sec-parser-state-stmt]).
* Added support for casts from `int` to `bool` (<<_casts>>).
* Added support for 0-width bitstrings and varbits (<<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>>).
* Clarified that `default_action` is `NoAction` if otherwise unspecified (Section [#sec-tables]).
* Clarified the types of expressions that may be used as indexes for header stacks (<<_operations_on_header_stacks>>).
* Clarified representation of Booleans in headers (<<_header_types>>).
* Clarified representation of empty types (<<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>>).
...
* Fixed several typos and inconsistencies in grammar (<<_appendix_p4_grammar>>).
* Eliminated annotations on `const` entries in grammar (<<_appendix_p4_grammar>>).

=== Summary of changes made in version 1.2.1, released June 11, 2020

* Added structure-value expressions (<<_operations_on_structure_valued_expressions>>).
* Added support for default values (<<_default_values>>).
* Added support for concatenating signed strings (<<_concatenation>>).
* Added key-value and list-structured annotations (<<_annotations>>).
* Added `@pure` and `@noSideEffects` annotations (<<_extern_function_method_annotations>>).
* Added `@noWarn` annotation (<<_no_warnings_annotation>>).
* Generalized typing for masks to allow serializable ``enum``s (<<_masks>>).
* Restricted the right operands of bit shifts involving
  arbitrary-precision integers to be constant and positive (<<_operations_on_arbitrary_precision_integers>>).
* Clarified copy-out behavior for `return` (<<_return_statement>>) and `exit` (<<_exit_statement>>)
  statements.
* Clarified semantics of invalid header stacks (<<_reading_uninitialized_values_and_writing_fields_of_invalid_headers>>).
* Clarified initialization semantics (<<_l_values>> and
  <<_calling_convention_call_by_copy_in_copy_out>>), especially for headers and local
  variables.
* Clarified evaluation order for table keys (<<_match_action_unit_execution_semantics>>).
* Fixed grammar to clarify parsing of right shift operator (`>>`),
  allow empty statements in parser (Section [#sec-parser-state-stmt]),
  and eliminate annotations on const entries (<<_entries>>).

=== Summary of changes made in version 1.2.0, released October 14, 2019

* Added `table.apply().miss` (<<_match_action_unit_invocation>>).
* Added `string` type (<<_strings>>).
* Added implicit casts from enum values (<<_operations_on_enum_types>>).
...
* Added `int` type (<<_arbitrary_precision_integers>>).
* Added error `ParserInvalidArgument` (<_variable_width_extraction>>, <<_skipping_bits>>).
* Clarified the significance of order of entries in `const entries` (<<_entries>>).
* Added methods to calculate header size (<<_operations_on_headers>>).

=== Summary of changes made in version 1.1.0, released November 26, 2017.

* Top-level functions (<<_function_declarations>>)
    - Functions may be declared at the top-level of a P4 program.
* Optional and named parameters (<<_calling_convention_call_by_copy_in_copy_out>>)
    - Parameters may be specified by name, with a default value, or designated as optional.
* `enum` representations (<<_operations_on_enum_types>>)
    - `enum` values to be specified with a concrete representation.
* Parser values sets (<<_parser_value_sets>>)
    - `value_set` objects for control-plane programmable `select` labels.
* Type definitions (<<_introducing_new_types>>)
    - New types may be introduced in programs.
* Saturating arithmetic (<<_operations_on_fixed_width_bit_types_unsigned_integers>>)
    - Saturating arithmetic is supported on some targets.
* Structured annotations (<<_annotations>>)
    - Annotations may be specified as lists of key-value pairs
* Globalname (<<_annotations_controlling_naming>>)
    - The reserved `globalname` annotation has been removed.
* Table `size` property (<<_size>>)
    - Meaning of optional `size` property for tables has been defined.
* Invalid headers (<<_operations_on_headers>>)
    - Clarified semantics of operations on invalid headers.
* Calling restrictions (<<_appendix_restrictions_on_compile_time-and_run_time_calls>>)
    - Added restrictions on kinds of values that may be passed as arguments to calls.
* Bitwise operator precedence (<<_appendix_p4_grammar>>)
    - Modified precedence conventions so that bitwise operators `&` `|` and `^` have higher precedence than relation operators `<` `>` `<=` `>=`.
* Computed bitwidths (<<_base_types>>)
...
//+---------------+:---------------------------------------------------+:-------------------------+
//| `atomic`      | specify atomic execution                           | <<_concurrency_model>>       |
//| `defaultonly` | action can only appear in the default action       | <<_annotations_on_the_table_action_list>> |
//| `hidden`      | hides a controllable entity from the control plane | <<_annotations_controlling_naming>>  |
//| `match`       | specify `match_kind` of a field in a `value_set`   | <<_parser_value_sets>> |
//| `name`        | assign local control-plane name                    | <<_annotations_controlling_naming>>  |
//| `optional`    | parameter is optional                              | <<_optional_parameter_annotations>> |
//| `tableonly`   | action cannot be a default_action                  | <<_annotations_on_the_table_action_list>> |
//| `deprecated`  | Construct has been deprecated                      | <<_deprecated_annotation>> |
//| `pure`        | pure function                                      | <<_extern_function_method_annotations>> |
//| `noSideEffects` | function with no side effects                    | <<_extern_function_method_annotations>> |
//| `noWarn`      | Has a string argument; inhibits compiler warnings  | <<_no_warnings_annotation>>       |
//|---------------|----------------------------------------------------|--------------------------|
...
(also described in the VSS example in <<_available_extern_blocks>>) for
...
instantiations (see <<_instantiations>>) of different types.
...
within a parser, control, or function (see <<_variables>> for more
...
called.  See <<_calling_convention_call_by_copy_in_copy_out>>.
...
enclosed within double quotes, as described in <<_string_literals>>.
